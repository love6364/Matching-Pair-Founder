...
# st.set_page_config(page_title="Diamond Pair Finder", layout="wide")

# st.title("💎 Diamond Pair Finder")
...
#                     pair_id = str(pair_no).zfill(2)

#                     df.at[i, "Pair_ID"] = f"{pair_id}A"
#                     df.at[j, "Pair_ID"] = f"{pair_id}B"
...
#         pair_col = df.columns.get_loc("Pair_ID") + 1

#         for row in range(2, len(df) + 2):

#             if worksheet.cell(row, pair_col).value:
...
#     st.success(
#         f"Total Pairs Found : {total_pairs}"
#     )
...
#             pair_number = str(pair_value)[:-1]

#             if pair_number not in pair_fill_map:

#                 color = generate_bright_color()

#                 pair_fill_map[pair_number] = PatternFill(
#                     start_color=color,
#                     end_color=color,
#                     fill_type="solid"
#                 )

#             fill = pair_fill_map[pair_number]
...
#         f"Paired rows highlighted in <b style='color:#c6a700'>yellow (A)</b> and "
#         f"<b style='color:#2b6cb0'>blue (B)</b> in the exported file. "
